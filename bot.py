...
def mark_group_notified_for_date(group, date_obj):
    ...
    # Если group уже есть и это не словарь, пересоздаём как словарь
    if group in data[today]:
        if not isinstance(data[today][group], dict):
            data[today][group] = {}
    
    # Если group ещё нет, создаём пустой словарь
...
def check_and_notify_new_replacements():
    ...
    for d in dates_to_check:
        ...
        # 2. Изменённые замены (группы, которые были и остались)
                    ...
                    for uid_str, data in users.items():
                        if data.get("group") == group:
                            ...
                            try:
                                ...
                                print(f"      ❌ Ошибка отправки {uid}: {e}")
                    # Отмечаем как уведомлённые на эту дату
                    mark_group_notified_for_date(group, d)
            ...
            for uid_str, data in users.items():
                if data.get("group") == group:
                    ...
                    try:
                        ...
                        print(f"      ❌ Ошибка отправки {uid}: {e}")
            # Для отмены не отмечаем как уведомлённые, чтобы в финальной рассылке отправили "нет замен" если нужно
...
def get_dates_to_check():
    ...
    if weekday == 4:  # пятница
        dates.append(today + timedelta(days=1))  # суббота
        dates.append(today + timedelta(days=3))  # понедельник
    elif weekday >= 5:  # суббота или воскресенье
        if weekday == 5:  # суббота
            dates.append(today + timedelta(days=2))  # понедельник
        else:  # воскресенье
            dates.append(today + timedelta(days=1))  # понедельник
    else:
...
def monitor_updates():
    while True:
        now = datetime.now()
        if 8 <= now.hour < 17 or (now.hour == 20 and now.minute < 20):
...
if __name__ == "__main__":
    ...
    # Ручной запуск с обработкой ошибок
    print("🔍 Ручная проверка...")
    try:
        print("📢 Отправляю тестовое сообщение...")
        bot.send_message(1526536345, "✅ Бот запущен и работает!")
        
        print("📢 Проверяю замены...")
        check_and_notify_new_replacements()
    except Exception as e:
        print(f"❌ Ошибка: {e}")
        bot.send_message(1526536345, f"❌ Ошибка при запуске: {e}")
